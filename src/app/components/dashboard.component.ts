  ...
  template: `
    <div class="dashboard-container">
      ...
      <div
        ...
        *ngIf="(categoriesStats$ | async)?.length! > 0"
      >
        <div class="section-header">
          <div class="section-title-wrapper">
            ...
            <p class="section-subtitle">
              Click any category to filter your birthdays
            </p>
          </div>
          <button
        ...
        <div class="categories-grid">
          <div
            *ngFor="let categoryStats of categoriesStats$ | async"
            ...
            <div class="category-stat-content">
              <div class="category-icon-section">
                ...
                <div
                  ...
                  *ngIf="isCategorySelected(categoryStats.id)"
                >
        ...
        <div class="filter-info" *ngIf="selectedCategory">
          <div class="filter-text">
            ...
            Showing
            {{ (birthdayService.filteredBirthdays$ | async)?.length }} birthdays
        ...
        <div
          class="filtered-dashboard-grid"
          *ngIf="
            selectedCategory &&
            (birthdayService.filteredBirthdays$ | async)?.length! > 0
          "
        >
          <div
            *ngFor="
              let birthday of getSortedFilteredBirthdays(
                birthdayService.filteredBirthdays$ | async
              )
            "
            class="filtered-birthday-card"
          >
      ...
      <div
        ...
        *ngIf="(birthdayService.birthdays$ | async)?.length! > 0"
      >
        <mat-card class="birthday-list-card">
          ...
          <mat-card-content>
            <div
              ...
              *ngIf="(allBirthdays$ | async)?.length! > 0"
            >
              <div
                *ngFor="
                  let birthday of allBirthdays$ | async;
                  trackBy: trackByBirthday
                "
              ...
              >
                ...
                <div class="dashboard-birthday-content">
                  ...
                  <div class="dashboard-birthday-details">
                    ...
                    <div
                      class="dashboard-birth-date-edit"
                      *ngIf="isEditing(birthday.id)"
                    >
                      <mat-icon class="small-icon">cake</mat-icon>
                      <input
                    ...
                    <div
                      ...
                      *ngIf="birthday.notes && !isEditing(birthday.id)"
                    >
                      <mat-icon class="small-icon">note</mat-icon>
                      {{ birthday.notes }}
                    </div>
                    <div
                      class="dashboard-notes-edit"
                      *ngIf="isEditing(birthday.id)"
                    >
                      <mat-icon class="small-icon">note</mat-icon>
                      <input
                  ...
                  <div
                    class="dashboard-category-edit"
                    *ngIf="isEditing(birthday.id)"
                  >
                    <mat-icon class="small-icon">category</mat-icon>
                    <select
                      [(ngModel)]="editingBirthdayData.category"
                      (change)="onEditInputChange(birthday)"
                      class="edit-category-select"
                    >
                      <option
                        *ngFor="let category of getBirthdayCategories()"
                        [value]="category.id"
                      >
                        {{ category.name }}
                      </option>
                ...
                <div class="dashboard-actions">
                  <div
                    ...
                    [class]="
                      getDaysChipClass(
                        birthdayService.getDaysUntilBirthday(birthday.birthDate)
                      )
                    "
                    *ngIf="!isEditing(birthday.id)"
                  >
                    <div class="days-circle-content">
                      <div class="days-number">
                        {{
                          getDaysText(
                            birthdayService.getDaysUntilBirthday(
                              birthday.birthDate
                            )
                          )
                        }}
        ...
        <mat-card class="chart-card">
          ...
          <mat-card-content>
            <div class="chart-container">
              <div
                *ngFor="let monthData of chartData$ | async"
                class="chart-bar"
              >
                <div class="bar-container">
                  <div
                    class="bar"
                    [style.height.%]="
                      getBarHeight(monthData.count, maxCount$ | async)
                    "
                    [class.current-month]="
                      monthData.monthIndex === currentMonth
                    "
                  ></div>
                  <div class="bar-value" *ngIf="monthData.count > 0">
                    {{ monthData.count }}
                  </div>
                </div>
  ...
  styles: [
    `
      ...
      .birthday-days-circle {
        ...
        &.red-alert {
          ...
          box-shadow: 0 0 35px rgba(255, 71, 87, 0.9),
            0 0 70px rgba(255, 71, 87, 0.5), 0 0 100px rgba(255, 71, 87, 0.2),
      ...
      @keyframes red-alert-attention {
        ...
        25% {
          ...
          box-shadow: 0 0 45px rgba(255, 71, 87, 1),
            0 0 90px rgba(255, 71, 87, 0.6), 0 0 120px rgba(255, 71, 87, 0.3);
        }
        50% {
          transform: scale(1.04) rotate(-2deg);
          box-shadow: 0 0 40px rgba(255, 71, 87, 0.95),
            0 0 80px rgba(255, 71, 87, 0.5), 0 0 110px rgba(255, 71, 87, 0.25);
        }
        75% {
          transform: scale(1.06) rotate(2deg);
          box-shadow: 0 0 42px rgba(255, 71, 87, 0.98),
            0 0 85px rgba(255, 71, 87, 0.55), 0 0 115px rgba(255, 71, 87, 0.28);
        }
      }
      ...
      @keyframes red-text-glow {
        ...
        50% {
          text-shadow: 0 2px 4px rgba(0, 0, 0, 0.4),
            0 0 12px rgba(255, 255, 255, 0.9), 0 0 20px rgba(255, 71, 87, 0.6);
        }
      }
...
export class DashboardComponent {
    ...
    this.categoriesStats$ = this.birthdayService.birthdays$.pipe(
      map((birthdays) => {
        const stats = BIRTHDAY_CATEGORIES.map((category) => ({
          ...category,
          count: birthdays.filter(
            (b) => (b.category || DEFAULT_CATEGORY) === category.id
          ).length,
    ...
    if (navigator.share) {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            if (blob) {
              const file = new File([blob], `${birthday.name}_birthday.jpg`, {
                type: 'image/jpeg',
              });
              navigator.share({
        ...
        .subscribe((birthdays) => {
          ...
          if (birthday) {
            const updatedBirthday = {
              ...
              name: this.editingBirthdayData.name?.trim() || birthday.name,
              notes:
                this.editingBirthdayData.notes?.trim() || birthday.notes || '',
              birthDate: this.editingBirthdayData.birthDate
                ? new Date(this.editingBirthdayData.birthDate)
                : birthday.birthDate,
  ...
  private autoSaveBirthday(birthday: any): void {
    if (
      this.editingBirthdayId === birthday.id &&
      (this.editingBirthdayData.name?.trim() || birthday.name)
    ) {
      const updatedBirthday = {
        ...birthday,
        name: this.editingBirthdayData.name?.trim() || birthday.name,
        ...
        category: this.editingBirthdayData.category || birthday.category,
        photo:
          this.editingBirthdayData.photo !== undefined
            ? this.editingBirthdayData.photo
            : birthday.photo,
        rememberPhoto:
          this.editingBirthdayData.rememberPhoto !== undefined
            ? this.editingBirthdayData.rememberPhoto
  ...
  private updateAllBirthdays(): void {
    this.allBirthdays$ = this.birthdayService.birthdays$.pipe(
      map((birthdays) => {
        ...
        let filteredBirthdays = [...birthdays].sort(
          (a, b) =>
            this.birthdayService.getDaysUntilBirthday(a.birthDate) -
            this.birthdayService.getDaysUntilBirthday(b.birthDate)
        );
